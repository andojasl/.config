		...
		this.panel.webview.onDidReceiveMessage(
            async (message) => {
                switch (message.command) {
                    ...
                    case 'savePreset':
                        try {
                            await savePreset(message.data.name, message.data, extensionPath);
						...
						currentDahuTheme = message.data;
						this.panel.webview.postMessage({ command: 'currentTheme', data: currentDahuTheme });
						this.panel.webview.postMessage({command: 'applyPreset', data: currentDahuTheme})
                        break;
					case 'applyPreset':
