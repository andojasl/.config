            ...
            async (message) => {
                switch (message.command) {
					...
					case 'getWorkbenchColors':
						...
						this.panel.webview.postMessage({command: 'presents', data: colors});
                    case 'savePreset':
                        try {
...
const getWorkbenchColors = () => {
	...
	console.log('Current Workbench Colors:', colorCustomizations, "\n");
...
export function activate(context: vscode.ExtensionContext) {
    ...
    // Retrieve and log the workbench colors when the extension is activated
    getWorkbenchColors();
