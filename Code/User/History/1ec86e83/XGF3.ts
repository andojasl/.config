...
@Component({
  ...
  imports: [CommonModule, ReactiveFormsModule, ColorSketchModule, FormsModule, ColorPickerComponent, ColorPickerModule,
      LibraryComponent, 
      EditSyntaxComponent, 
...
export class AppComponent implements AfterViewInit{
  ...
  scrollToLibrary() {
    const libraryComponent = document.getElementById('libraryComponentId');

    if(libraryComponent){
    this.libraryComponent.nativeElement.scrollIntoView({ behavior: 'smooth', block: 'start'});
    } else{
      console.log("No library nanana");
    }
  }
}
