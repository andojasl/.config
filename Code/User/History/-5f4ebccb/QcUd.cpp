...
bool readButton() {
    ...
    if (i2cMutex != NULL && xSemaphoreTake(i2cMutex, portMAX_DELAY)) {
        Wire.beginTransmission(ADDR2);
        Wire.write(0x12); // GPIO register (assuming GPIOA)
        Wire.endTransmission();
        Wire.requestFrom(ADDR2, 1);
        if (Wire.available()) {
            int value = Wire.read();
        ...
        if ((xTaskGetTickCount() - lastDebounceTime) > pdMS_TO_TICKS(50)) {
            if (currentButtonState != lastButtonState) {
                ...
                if (currentButtonState) {
                    ...
                    xSemaphoreGive(pedRequestSemaphore); // Signal the pedestrian request
                } else {
...
void trafficLightTask(void *parameter) {
    ...
    while (true) {
        ...
        while (xTaskGetTickCount() - startTime < pdMS_TO_TICKS(10000)) {
            if (xSemaphoreTake(pedRequestSemaphore, 0) == pdTRUE) {
                buttonPressed = true;
                break;
            }
