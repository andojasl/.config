...
class WebviewPanel {
  ...
  public static createOrShow(extensionPath: string, view: string) {
    ...
    const panel = vscode.window.createWebviewPanel(
      ...
      {
        enableScripts: true,
        localResourceRoots: [vscode.Uri.file(path.join(extensionPath, 'demo-routing/dist/ext-src'))]
      }
    );
  ...
  private getWebviewContent(view: string): string {
      const appDistPath = path.join(this._extensionPath, 'demo-routing', 'dist', 'ext-src');
